...
class PreProcessing():
    def __init__(self, csv_dir) -> None:
        ...
        if os.path.isfile(dict_path):
            ...
            df = pd.read_csv(os.path.join(csv_dir, "train.csv"))
            self.label_dict = self.create_dict(df)
        
        self.train_df = self.create_encoded_df(os.path.join(csv_dir, "train.csv"))
        self.test_df = self.create_encoded_df(os.path.join(csv_dir, "test.csv"))
